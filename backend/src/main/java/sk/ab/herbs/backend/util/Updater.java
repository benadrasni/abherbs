...
//            }
    ...
    private static void luontoportti() {

        Map<String, String> labels = new HashMap<>();
        Map<String, List<String>> aliases = new HashMap<>();

        try {
//            Document docList = Jsoup.connect("http://www.luontoportti.com/suomi/fi/kukkakasvit/?list=9").get();
//
...
//                        for(Element li : lis) {
//                            if (li.text().startsWith("Nimi myös:")) {
...
//            }
            ...
            File namefile = new File(PATH + "finames.csv");
            ...
            while(scan.hasNextLine()) {
                ...
                String valueLabel = plant.getLabel().get("fi");
                ...
                ArrayList<String> valueAlias = plant.getNames().get("fi");
                if (valueAlias != null) {
                ...
                if (label != null) {
                    if (!label.equals(valueLabel)) {
                        callCloud = herbCloudClient.getApiService().update(plantLine[0], "label_fi", label, "replace", "string");
                        callCloud.execute();
                    }
                } else {
                    if (valueLabel == null) {
                ...
                if (aliasSb.length() > 0) {
                    ...
                    if (!s.equals(valAlias.toString())) {
                        callCloud = herbCloudClient.getApiService().update(plantLine[0], "alias_fi", s, "replace", "list");
                        callCloud.execute();
                    }
                }
